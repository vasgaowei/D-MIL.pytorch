...
from detectron2.layers import Conv2d, Linear, ShapeSpec, batched_nms, cat, nonzero_tuple
from detectron2.modeling.box_regression import Box2BoxTransform
from detectron2.structures import Boxes, Instances
from detectron2.utils.events import get_event_storage

from wsl.layers import pcl_loss
from wsl.modeling.roi_heads.third_party.dmil import DMIL
from wsl.modeling.roi_heads.third_party.pcl import PCL
from wsl.modeling.roi_heads.third_party.dmil import PCLV1
from wsl.modeling.roi_heads.third_party.dmil import DMILV1
from wsl.modeling.roi_heads.third_party.dmil import PCL_Losses
...
class DISWSDDNOutputs(object):
    ...
    def _log_accuracy(self):
        ...
        pred_classes = ((self.pred_class_logits[0] + self.pred_class_logits[1] / 2.)).argmax(dim=1)
    ...
    def discrepancy_l1_loss(self):
        _, _, _, _, scores_det_0, scores_det_1 = self.pred_class_logits
        probs_det_0 = F.softmax(scores_det_0, dim=0)
        probs_det_1 = F.softmax(scores_det_1, dim=0)
        
        num_dets, _ = probs_det_0.shape
        mask = self.gt_classes_img_oh.repeat(num_dets, 1)
        probs_det_minus = - torch.abs(probs_det_0 - probs_det_1)
        probs_det_minus = probs_det_minus * mask
        
        loss_dis = torch.mean(probs_det_minus, dim=0)
        loss_dis = torch.sum(loss_dis) / self.gt_classes_img_oh.sum()
    ...
    def binary_cross_entropy_loss(self):
        ...
        """
        self._log_accuracy()
        pred_class_img_logits = self.predict_probs_img()
        
        reduction = "mean" if self.mean_loss else "sum"
        cls_loss0 = 0.0001 * F.binary_cross_entropy(
            pred_class_img_logits[0], self.gt_classes_img_oh, reduction=reduction
        ) / self.gt_classes_img_oh.size(0)
        
        cls_loss1 = 0.0001 * F.binary_cross_entropy(
            pred_class_img_logits[1], self.gt_classes_img_oh, reduction=reduction
        ) / self.gt_classes_img_oh.size(0)
        
        cls_loss0_dmil = F.binary_cross_entropy(
            pred_class_img_logits[2], self.gt_classes_img_oh, reduction=reduction
        ) / self.gt_classes_img_oh.size(0)
        
        cls_loss1_dmil = F.binary_cross_entropy(
            pred_class_img_logits[3], self.gt_classes_img_oh, reduction=reduction
        ) / self.gt_classes_img_oh.size(0)
        
        cls_loss = cls_loss0 + cls_loss1 + cls_loss0_dmil + cls_loss1_dmil
        
        return cls_loss

    def predict_probs_img(self):
        if not hasattr(self, "pred_class_img_logits"):
            self.pred_class_img_logits_0 = cat(
                [
                    torch.sum(xx, dim=0, keepdim=True)
                    for xx in self.pred_class_logits[0].split(self.num_preds_per_image, dim=0)
                ],
                dim=0,
            )
            self.pred_class_img_logits_0 = torch.clamp(
                self.pred_class_img_logits_0, min=1e-6, max=1.0 - 1e-6
            )
            
            self.pred_class_img_logits_1 = cat(
                [
                    torch.sum(xx, dim=0, keepdim=True)
                    for xx in self.pred_class_logits[1].split(self.num_preds_per_image, dim=0)
                ],
                dim=0,
            )
            self.pred_class_img_logits_1 = torch.clamp(
                self.pred_class_img_logits_1, min=1e-6, max=1.0 - 1e-6
            )
            
            self.pred_class_img_logits_0_dmil = cat(
                [
                    torch.sum(xx, dim=0, keepdim=True)
                    for xx in self.pred_class_logits[2].split(self.num_preds_per_image, dim=0)
                ],
                dim=0,
            )
            self.pred_class_img_logits_0_dmil = torch.clamp(
                self.pred_class_img_logits_0_dmil, min=1e-6, max=1.0 - 1e-6
            )
            
            self.pred_class_img_logits_1_dmil = cat(
                [
                    torch.sum(xx, dim=0, keepdim=True)
                    for xx in self.pred_class_logits[3].split(self.num_preds_per_image, dim=0)
                ],
                dim=0,
            )
            self.pred_class_img_logits_1_dmil = torch.clamp(
                self.pred_class_img_logits_1_dmil, min=1e-6, max=1.0 - 1e-6
            )
            
            
            self.pred_class_img_logits = [self.pred_class_img_logits_0, self.pred_class_img_logits_1, self.pred_class_img_logits_0_dmil, self.pred_class_img_logits_1_dmil]
    ...
    def losses(self):
        ...
        losses = {"loss_cls": self.binary_cross_entropy_loss()}
        losses.update({'loss_dis': self.discrepancy_l1_loss()})
        return losses
...
class DISWSDDNOutputLayers(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.cls_0 = Linear(input_size, num_classes)
        self.det_0 = Linear(input_size, num_classes)
        
        self.cls_1 = Linear(input_size, num_classes)
        self.det_1 = Linear(input_size, num_classes)

        # nn.init.normal_(self.cls.weight, std=0.01)
        # nn.init.normal_(self.det.weight, std=0.01)
        nn.init.xavier_uniform_(self.cls_0.weight)
        nn.init.xavier_uniform_(self.det_0.weight)
        for l in [self.cls_0, self.det_0]:
            nn.init.constant_(l.bias, 0)
            
        nn.init.xavier_uniform_(self.cls_1.weight)
        nn.init.xavier_uniform_(self.det_1.weight)
        for l in [self.cls_1, self.det_1]:
            nn.init.constant_(l.bias, 0)
        ...
        if isinstance(loss_weight, float):
            loss_weight = {"loss_cls": loss_weight, "loss_box_reg": loss_weight, "loss_dmil": loss_weight}
    ...
    def forward(self, x, proposals=None):
        ...
        if proposals is None:
            scores_cls_0 = self.cls_0(x)
            scores_det_0 = self.det_0(x)
            
            scores_cls_1 = self.cls_1(x)
            scores_det_1 = self.det_1(x)
            
            scores_det = scores_det_0 + scores_det_1
            
            scores_0 = F.softmax(scores_cls_0, dim=1) * F.softmax(scores_det_0, dim=0)
            scores_1 = F.softmax(scores_cls_1, dim=1) * F.softmax(scores_det_1, dim=0)
            
            scores_0_dmil = F.softmax(scores_cls_0, dim=1) * F.softmax(scores_det, dim=0)
            scores_1_dmil = F.softmax(scores_cls_1, dim=1) * F.softmax(scores_det, dim=0)
            scores = [scores_0, scores_1, scores_0_dmil, scores_1_dmil, scores_det_0, scores_det_1]
        else:
            ...
            #scores = cat(
            #    [
            #        F.softmax(self.cls(xx), dim=1) * F.softmax(self.det(xx), dim=0)
            #        for xx in x.split(num_preds_per_image, dim=0)
            #    ],
            #    dim=0,
            #)
        ...
        proposal_deltas = torch.zeros(
            ...
            dtype=scores_0.dtype,
            device=scores_0.device,
            requires_grad=False,
        )
    ...
    def predict_probs_img(self, predictions, proposals):
        ...
        pred_class_img_logits = cat(
            [
                ...
                for score in ((scores[0] + scores[1]) / 2.).split(num_inst_per_image, dim=0)
            ],
            dim=0,
        )
...
class WSDDNOutputLayers(nn.Module):
    ...
    def forward(self, x, proposals=None):
        ...
        if proposals is None:
            scores = F.softmax(self.cls(x), dim=1) * F.softmax(self.det(x), dim=0)
        else:
            num_preds_per_image = [len(p) for p in proposals]
            scores = cat(
                [
...
class OICROutputLayers(nn.Module):
    ...
    # TODO: move the implementation to this class.
    def losses(self, predictions, proposals, proposal_weights):
        """
        Args:
            predictions: return values of :meth:`forward()`.
            proposals (list[Instances]): proposals that match the features that were used
    ...
    def losses_dmil(self, predictions, proposals, last_scores, gt_classes_img_oh):
        """
        Args:
            predictions: return values of :meth:`forward()`.
            proposals (list[Instances]): proposals that match the features that were used
...
class DMILOutputsV1(object):
    ...
    def dmil_loss(self):
        ...
        pcl_output = DMILV1(boxes, self.last_score, im_labels, refine[0].clone().detach())

        refine_loss = self.pcl_losses(boxes.copy(), im_labels, refine[0], pcl_output)
        return refine_loss
    ...
    def pcl_loss(self):
        ...
        pcl_output = PCLV1(boxes.copy(), self.last_score, im_labels.copy())
    ...
    def losses(self):
        ...
        """
        if not self.has_reg:
            return {"loss_cls" + self.refine_k: self.dmil_loss()}
        return {
            "loss_cls" + self.refine_k: self.dmil_loss(),
            "loss_box_reg" + self.refine_k: self.box_reg_loss(),
        }
...
class DMILOutputs(object):
    ...
    def dmil_loss(self):
        ...
        pcl_output = DMIL(boxes, self.last_score, im_labels, refine[0].clone().detach())
...
class GAMOutputs(object):
    def __init__(self, pred_class_img_logits, mean_loss, gt_classes_img_oh):
        self.pred_class_img_logits = pred_class_img_logits
        self.gt_classes_img_oh = gt_classes_img_oh
        self.mean_loss = mean_loss
    ...
    def binary_cross_entropy_with_logits(self):
        ...
        return F.binary_cross_entropy_with_logits(
            self.pred_class_img_logits, self.gt_classes_img_oh, reduction=reduction
        )

    def losses(self):
        return {"loss_gam": self.binary_cross_entropy_with_logits()}


class GAMOutputLayers(nn.Module):
    def __init__(self, in_channels, num_classes):
        super(GAMOutputLayers, self).__init__()

        self.conv_6 = Conv2d(in_channels, in_channels, kernel_size=1, padding=0, bias=True)

        # kernel_size ?
        self.conv_7 = Conv2d(in_channels, num_classes, kernel_size=1, padding=0, bias=True)

        weight_init.c2_msra_fill(self.conv_6)
        weight_init.c2_msra_fill(self.conv_7)
    ...
    def forward(self, x):
        ...
        a.squeeze_(2).squeeze_(2)

        return x, a


class ATTOutputs(object):
    """
    ...
    def __init__(
        ...
        self.num_classes = self.pred_class_logits.shape[1]
    ...
    def binary_cross_entropy_loss_with_logits(self):
        """
        Compute the softmax cross entropy loss for box classification.
    ...
    def losses(self):
        ...
        return {
            ...
            "loss_cls": self.binary_cross_entropy_loss_with_logits()
        }
...
class ATTOutputLayers(nn.Module):
    """
    Two linear layers for predicting Fast R-CNN outputs:
      (1) proposal-to-detection box regression deltas
      (2) classification scores
    ...
    def __init__(self, input_size, num_classes, cls_agnostic_bbox_reg, box_dim=4):
        ...
        """
        super(ATTOutputLayers, self).__init__()
        ...
        oplists = [nn.Linear(input_size, 1) for i in range(num_classes)]
        self.cls_fcs = nn.ModuleList(oplists)
        for op in self.cls_fcs:
    ...
    def forward(self, x):
        ...
        # print(torch.sum(scores[:, 0].view(-1, 1) * x, dim=0).size())
        fcs_scores = cat(
            [
                op(torch.sum(scores[:, i].view(-1, 1) * x, dim=0))
                for i, op in enumerate(self.cls_fcs)
            ],
            dim=0,
        ).view(1, -1)
        ...
        return scores, proposal_deltas, fcs_scores
