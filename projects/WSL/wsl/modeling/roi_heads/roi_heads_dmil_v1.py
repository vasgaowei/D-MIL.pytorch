...
from wsl.modeling.roi_heads.fast_rcnn import OICROutputLayers, DISWSDDNOutputLayers
...
class DMILROIHeadsV1(ROIHeads):
    ...
    def _init_box_head(cls, cfg, input_shape):
        ...
        box_predictor = DISWSDDNOutputLayers(cfg, box_head.output_shape)
    ...
    def _forward_box(
        ...
        if self.training:
            ...
            prev_pred_scores_list = predictions[0][:2]
            prev_pred_scores = [prev_pred_scores_list[0].detach(), prev_pred_scores_list[1].detach()]
            for k in range(self.refine_K):
                predictions_k = self.box_refinery[k](box_features)

                if k == 0:
                    losses_k = self.box_refinery[k].losses_dmil(
