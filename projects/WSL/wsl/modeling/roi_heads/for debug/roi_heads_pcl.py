...
class PCLROIHeads(ROIHeads):
    ...
    def _forward_box(
        ...
        """
        print('_forward_box self.box_in_features', self.box_in_features)
        print('_forward_box features', len(features))
        print('_forward_box features', features.keys())
        features = [features[f] for f in self.box_in_features]
        box_features = self.box_pooler(features, [x.proposal_boxes for x in proposals])
        print('proposals', len(proposals))
        #print('proposals[0]', proposals[0])
        print('proposals[0]', len(proposals[0]))
        #print('proposals.proposal_boxes', proposals[0].proposal_boxes)
        print('_forward_box features', len(features))
        print('_forward_box box_features', len(box_features))
        print('_forward_box features', features[0].shape)
        print('_forward_box box_features', box_features[0].shape)
        ...
        print('_forward_box box_features', box_features.shape)
        predictions = self.box_predictor(box_features, proposals)
        # del box_features
        print('_forward_box predictions', predictions[0].shape)
        print('_forward_box gt_classes_img_oh', self.gt_classes_img_oh.shape)
        print('_forward_box gt_classes_img_oh', self.gt_classes_img_oh)
        if self.training:
            ...
            print('_forward_box self.pred_class_img_logits', self.pred_class_img_logits.shape)
            prev_pred_scores = predictions[0].detach()
            print('_forward_box prev_pred_scores', prev_pred_scores.shape)
            for k in range(self.refine_K):
