...
if __name__ == '__main__':
  ...
  for i in range(num_images):
      ...
      for j in range(imdb.num_classes):
          if labels[0,j] > 0:
              ...
              _, _, mutual_iou = iou_other_self_mutual(np.expand_dims(gt_box_1,0), gt_box_2)
              if mutual_iou[0] < 0.2:
                  inconsistent_proposal_num += 1
  ...
  rate = inconsistent_proposal_num / all_proposal_num
  if not os.path.exists('re_data_discrepancy_0.2.txt'):
      open('re_data_discrepancy_0.2.txt', 'w').close()
  with open('re_data_discrepancy_0.2.txt', 'a+') as f:
      f.write(str(rate)+'\n')
  print("test time: %0.4fs" % (end - start))
  print('all proposal:{:.1f}, inconsistent proposal:{:.1f}, rate is {:.6f}'\
        .format(all_proposal_num, inconsistent_proposal_num, rate))