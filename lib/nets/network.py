...
class Network(nn.Module):
  ...
  def _add_score_summary(self, key, tensor):
    return tb.summary.histogram('SCORE/' + key + '/scores', torch.tensor(np.zeros([2000, 21]), dtype=torch.float32), bins='auto')
  ...
  def forward(self, image, im_info, gt_boxes=None, ss_boxes=None, mode='TRAIN'):
    ...
    rois, cls_prob, bbox_pred = self._predict(ss_boxes)
  ...
  def test_image(self, image, im_info, ss_boxes):
    self.eval()
    with torch.no_grad():
      self.forward(image, im_info, None, ss_boxes, mode='TEST')
